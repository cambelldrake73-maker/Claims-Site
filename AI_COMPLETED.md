...
- Create a new folder services/claim_ingestion/parser_router and add a TypeScript/Node.js scaffold (index.ts) that exposes a parseFile(fileId: string, metadata: Record<string,any>) -> Promise<RawClaimBundle> and register it in the service registry bootstrap.
...
- Add canonical_claim_schema JSON Schema file schemas/canonical-claim.schema.json and TypeScript model types under libs/schemas/, plus ajv validator module libs/validators/canonicalValidator.ts; include schema versioning metadata and tests.
