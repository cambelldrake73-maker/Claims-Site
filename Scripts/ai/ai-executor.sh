...
EOF
    ...
    echo "Passive mode: skipping status API scaffolding."
    ...
    echo "Passive mode: use the live claims API directly and log the response."
elif echo "$TASK" | grep -iq "mock.*claims.*json"; then

    echo "Passive mode: skipping mock claims payload generation."
    ...
    echo "AI adding neutral claim badge styling..."
    ...
    if ! grep -q "AI improvement: claim status color mapping" claims.html; then
        {
            ...
            echo "<style>"
            echo ".status-badge{background:#eef2ff;color:#3730a3;}"
            echo "</style>"
        } >> claims.html
    else
    ...
    echo "AI adding passive dashboard logging note..."

    if ! grep -q "AI improvement: dashboard summary metrics" dashboard.html; then
        {
            echo ""
            echo "<!-- AI improvement: dashboard summary metrics -->"
            echo "<script>fetch('/api/claims').then(r => r.json()).then(data => console.log('Dashboard API payload:', data)).catch(err => console.error('Dashboard API error:', err));</script>"
        } >> dashboard.html
    else
...
elif echo "$TASK" | grep -iq "claim metadata field to mock claim payloads"; then

    echo "Passive mode: skipping mock claim metadata enrichment."

    if [ -f services/claim_ingestion_api/mock_data/claims.json ]; then
        echo "Mock claims payload already present."
...
EOF
    ...
    if ! grep -q "AI improvement: review queue api rendering" review-queue.html; then
        {
            ...
            echo "  console.log('Review queue API payload:', data);"
            echo "  list.innerHTML = data.claims.map(c => `<div class=\"queue-item\">${c.id}</div>`).join('');"
            echo "}"
    ...
    echo "Passive mode: skipping submission lifecycle scaffolding."
      ...
      <aside class="panel">
        ...
        <div class="detail-list">
          <div>
            ...
            Review queued claims using the live API response.
          </div>
          <div>
            <strong>Focus</strong>
            Inspect the payload returned by the review queue API.
          </div>
          <div>
            <strong>Goal</strong>
            Keep the page aligned with backend data and avoid local decision logic.
          </div>
        </div>
      </aside>
    </div>
          ...
          return `
            <div class="queue-item">
              ...
              <div class="queue-meta">
                ...
                Queue item loaded from API<br>
                API payload available in console<br>
...
EOF
    else
        {
            ...
            echo "<div class=\"claim-field\">Claim detail refreshed $(date +%s)</div>"
        } >> claim-detail.html
    fi
elif echo "$TASK" | grep -iq "claim status tracking system"; then

    echo "Passive mode: skipping status tracking scaffolding."
...
EOF
    ...
    echo "AI handling dashboard task with passive API logging..."
...
async function generateClaimSummary(claim) {
  const system = "You summarize claim API payloads for internal workflow use.";
...
elif echo "$TASK" | grep -Eiq "claim explanation endpoint using Claude provider"; then

    echo "AI building claim explanation endpoint..."
...
router.post('/api/claims/explanation', async (req, res) => {
  try {
    ...
    const system = "You explain claim API payloads for internal claim recovery teams.";
    const prompt = `Explain this claim payload and summarize the available details:\n${JSON.stringify(claim, null, 2)}`;
...
elif echo "$TASK" | grep -Eiq "GET /api/claims/intelligence|recovery trends|performance metrics|claims intelligence"; then
...
EOF
    ...
    echo "Passive mode: skipping batch normalize scaffolding."
else
